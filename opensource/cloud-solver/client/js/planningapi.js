planningapi = (function () {
  ...
  function solve() {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
        domain: problems[prob_index].domain_url,
      ...
      .done(function (res) {
        ...
        if (res["status"] === "ok") {
          ...
          $.each(res.result.plan, function (index, val) {
            ...
            else items.push("<li>" + val + "</li>");
          });
          $("#solution").html("<ol>" + items.join("") + "</ol>");
          $("#planner_output").html(
            "<pre>" + res.result.output.replace("\n", "<br/>") + "</pre>"
          );
        } else {
          progressbar_error();
          if (typeof res.result.killed != "undefined")
            ...
            $("#planner_output").html(
              "<pre>" + res.result.error.replace("\n", "<br/>") + "</pre>"
            );
          else
            $("#planner_output").html(
              "<pre>" + JSON.stringify(res.result, null, 3) + "</pre>"
            );
        }
      })
  ...
  function domain_change() {
    ...
    $.getJSON(
      API_URL + "problems/" + domains[dom_index].domain_id,
      function (data) {
        data.result.sort(function (a, b) {
          return a.problem.toLowerCase() > b.problem.toLowerCase();
        });
        problems = data.result;
        var items = [];
        $.each(problems, function (index, val) {
          items.push(
            "<option value = " + index + ">" + val.problem + "</option>"
          );
        });
  ...
  function _init() {
    $.getJSON(API_URL + "collections", function (data) {
      ...
      $.each(collections, function (index, val) {
        items.push(
          "<option value = " + index + ">" + val.collection_name + "</option>"
        );
      });
    ...
    $.getJSON(API_URL + "domains", function (data) {
      data.result.sort(function (a, b) {
        return a.domain_name.toLowerCase() > b.domain_name.toLowerCase();
      });
      domains = data.result;
      var items = [];
      $.each(domains, function (index, val) {
