...
class TestGrammarName(TestCase):
    ...
    def test_name_parses_kebab_case(self):
        try:
            parser = Lark(
                grammar=self.grammar, import_paths=self.import_path, parser=self.parser
            )
    ...
    def test_name_parsing_fails(self):
        ...
        self.assertRaises(
            ...
            parser.parse,
            """space separated""",
            msg="failed to raise on space separator",
        )
        self.assertRaises(
            Exception,
            parser.parse,
            """(not-char""",
            msg="failed to raise on non char in word",
        )
        self.assertRaises(
            Exception,
            parser.parse,
            """1not-num""",
            msg="failed to raise on num in word",
        )
