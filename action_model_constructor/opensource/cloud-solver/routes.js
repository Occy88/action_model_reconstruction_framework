...
module.exports = function (app) {
  ...
  */
  ...
  app.get("/solve", function (req, res) {
    ...
    tmp.dir(
      { prefix: "solver_planning_domains_tmp_", unsafeCleanup: true },
  ...
  app.post("/solve", function (req, res) {
    ...
    if (app.check_for_throttle(req)) {
      res.end(
        JSON.stringify({ status: "error", result: "Server busy..." }, null, 3)
      );
      return;
    } else if (!app.get_lock()) {
      app.server_in_contention();
    ...
    tmp.dir(
      ...
      function _tempDirCreated(dirErr, path, cleanupCallback) {
        var cleanupAndRespond = function (error, result) {
          ...
          if (error)
            res.end(
              JSON.stringify({ status: "error", result: message }, null, 3)
            );
          else
  ...
  app.post("/validate", function (req, res) {
    ...
    if (typeof req.body.plan == "undefined") {
      res.end(
        JSON.stringify(
          { status: "error", result: "Error: No plan to verify" },
          null,
          3
        )
      );
