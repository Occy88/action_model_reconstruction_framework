...
struct DepotDescriptor {
    enum ProblemType {
        STRIPS, NUMERIC, SIMPLETIMED, TIMED
    };
    int numDepots;
    int numDistributors;
    int numTrucks;
    ...
    DepotDescriptor(int dps, int dsts, int trks, int plts, int hsts, int crts, ProblemType tp = DepotDescriptor::STRIPS,
                    int mxwt = 0, int mxcp = 0) :
...
private:
    ...
    void location(std::ostream &o, int i) const {
        if (i < numDepots) {
            o << "depot" << i;
        } else {
    ...
    void location(std::ostream &o, const locatable &l) const {
...
public:
    Depot(unsigned int s, const DepotDescriptor &d) :
        ...
        m = new Map(locs);
        int numPallets = std::max(d.numPallets, locs);
        int numHoists = std::max(d.numHoists, locs);
